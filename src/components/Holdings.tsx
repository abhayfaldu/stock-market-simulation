...
export default function Holdings() {
  ...
  if (loading.holdings) {
    return (
      <div className='border border-gray-800 rounded-lg overflow-hidden p-4'>
              ...
              <div className='flex justify-between items-start mb-2'>
                <div>
                  ...
                  <p className='text-sm text-gray-400'>
                    Quantity: <span className='text-gray-200'>{holding.quantity}</span> | Avg Price:
                    <span className='text-gray-200'>₹{holding.averagePrice?.toFixed(2)}</span>
                  </p>
                </div>
                <div className='text-right'>
                  ...
                  <p className={`text-sm ${profit >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                    {profit >= 0 ? '+' : ''}
                    {profitPercentage}% ({profit >= 0 ? '+' : ''}₹{profit.toFixed(2)})
                  </p>
                </div>
              </div>
              <div className='flex justify-between text-sm text-gray-400'>
                <span>Current Price: <span className='text-gray-200'>₹{stock.price.toFixed(2)}</span></span>
                <span>Invested: <span className='text-gray-200'>₹{investedValue.toFixed(2)}</span></span>
              </div>
            </div>
          )
        })}
